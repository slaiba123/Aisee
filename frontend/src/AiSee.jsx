...
export default function AiSee() {
  ...
  const TEAM_ITEMS = [
    ...
    { name: "Laiba Mushtaq", img: laiba },
    { name: "Sehrish Ahmed", img: sehrish },
  ];

  const dark = toggled;

  return (
    <div className={`font-poppins min-h-screen ${dark ? 'bg-black text-white' : 'bg-white text-gray-900'}`}>
      ...
      >
        ...
        <nav className="relative z-10 flex items-center justify-between px-5 md:px-14 py-5">
          ...
          <div className="flex items-center">
            ...
            <ul className="hidden md:flex gap-6 lg:gap-10 text-white text-sm font-medium">
              {NAV_LINKS.map((l) => (
                <li key={l.label} className="cursor-pointer hover:text-gray-300 transition-colors tracking-wide">
                  <a
                    href={l.href}
                    onClick={(e) => {
          ...
          >
            ...
            <span
              className={`absolute top-0.5 w-6 h-6 rounded-full bg-black shadow-md transition-all duration-300 z-20 ${dark ? "left-7" : "left-0.5"}`}
            />
          </button>
        </nav>

        {/* Mobile slide-down menu — always rendered, animated via max-height + opacity */}
        <div
          ...
          <div className="bg-black/95 backdrop-blur-sm border-t border-gray-800 px-6 py-5 flex flex-col gap-1">
            {NAV_LINKS.map((l, i) => (
              <a
                key={l.label}
                href={l.href}
                className="text-white text-base tracking-wide py-2.5 border-b border-gray-800/60 last:border-0 hover:text-gray-300 transition-colors"
                style={{
                  opacity: menuOpen ? 1 : 0,
        ...
        <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-4 pb-16">
          <div className="w-full max-w-xs sm:max-w-md md:max-w-2xl lg:max-w-4xl">
            <img className="w-full" src={logo} alt="AiSee Logo" />
          </div>
        </div>
      </section>

      {/* ── ABOUT ── */}
      <section id="about" className="py-16 md:py-20 px-5 mt-8 md:mt-16">
        <div className="max-w-2xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold mb-6 md:mb-8 tracking-wider">About Our Product</h2>
          <p className="leading-relaxed text-sm sm:text-base md:text-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 sm:gap-8 md:gap-12 mt-6 md:mt-16">
            {WHY_ITEMS.map((item) => (
              <div key={item.title} className="flex flex-col items-center gap-4 cursor-pointer">
                ...
                <h3 className="text-lg md:text-xl font-bold tracking-wider whitespace-nowrap">{item.title}</h3>
      ...
      {/* ── HOW IT WORKS ── */}
      <section id="working" className={`${dark ? 'bg-black text-white' : 'bg-white text-gray-900'} py-14 px-5`}>
        <div className="max-w-lg mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold mb-10 md:mb-14 text-center tracking-wider">How It Works</h2>
          <div className="flex flex-col gap-8 md:gap-10">
            {HOW_STEPS.map((s) => (
              <div key={s.step} className="flex items-center gap-4 md:gap-5">
                <div className={`shrink-0 w-20 h-20 md:w-32 md:h-32 rounded-full border-2 ${!dark ? 'bg-black text-white' : 'bg-white text-gray-900'} border-gray-900 flex items-center justify-center`}>
                  {s.icon}
                </div>
                <div>
                  <h3 className="font-bold text-lg md:text-xl mb-1 tracking-wider">{s.step}</h3>
                  <p className="text-sm md:text-base leading-relaxed">{s.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── MEET THE TEAM ── */}
      <section className={`${dark ? 'bg-black text-white' : 'bg-white text-gray-900'} py-14 px-5`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-6 lg:gap-40 mt-6 md:mt-10 justify-items-center">
            {TEAM_ITEMS.map((member) => (
              <div key={member.name} className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm md:text-base font-semibold text-center tracking-wide">{member.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className={`${dark ? 'bg-black text-white' : 'bg-white text-gray-900'} py-14 px-5`}>
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold mb-10 md:mb-14 tracking-wider">Supervised By</h2>
          <div className="flex flex-col items-center gap-3">
            <div className={`w-40 h-40 sm:w-52 sm:h-52 md:w-64 md:h-64 rounded-full overflow-hidden border-4 ${dark ? 'border-white' : 'border-gray-300'} bg-gray-200 hover:border-gray-500 cursor-pointer transition-all`}>
              <img
                src={miss}
                alt="Dr. Majida Kazmi"
                className="w-full h-full object-cover"
            ...
            <p className="text-base md:text-lg font-semibold tracking-wide">Dr. Majida Kazmi</p>
          </div>
        </div>
      </section>
      ...
      <section className={`${dark ? 'bg-black text-white' : 'bg-white text-gray-900'} py-14 px-5`}>
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-bold mb-10 md:mb-14 tracking-wider">In Collaboration With</h2>
          <div className="flex flex-col items-center gap-3">
            <div className={`w-full overflow-hidden rounded-lg border-4 ${dark ? 'border-white' : 'border-gray-300'} hover:border-gray-500 cursor-pointer transition-all`}>
              <img
                src={idaRieu}
            ...
            <p className="text-base md:text-lg font-semibold tracking-wide mt-1">Ida Rieu School For Blind And Deaf</p>
          </div>
        </div>
      </section>
